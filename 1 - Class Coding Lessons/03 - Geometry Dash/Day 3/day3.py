...
def reset_game():
    global player_x, player_y, player_velocity, on_ground, obstacles, platforms, score, game_over, time_up, start_time, line_obstacle
    player_x = 100
    player_y = HEIGHT - 100 - player_height
    player_velocity = 0
    on_ground = True
...
while running:
    ...
    # Check collision with line obstacle (if jumping)
    if line_obstacle and player_rect.colliderect(pygame.Rect(line_obstacle[0][0], line_obstacle[0][1] - 10, 100, 10)):
        if player_velocity < 0:  # If the player is jumping (moving up)
            game_over = True
    ...
    draw_player()
    draw_obstacles()
    draw_line_obstacle()  # Draw the line obstacle
    draw_ground()
    draw_waves()
    draw_score()
