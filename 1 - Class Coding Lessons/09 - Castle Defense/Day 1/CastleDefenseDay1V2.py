...
enemy_types = [
    ...
    {"color": YELLOW, "health": 100, "speed": 0.8, "reward": 30, "size": 15}
]
...
def spawn_enemy():
    # Accesses and modifies these global variables (defined around line 50-60):
    # - enemies_in_wave: Tracks total enemies spawned in current wave (line 56)
    # - enemies_remaining: Counts live enemies (line 57)
    global enemies_in_wave, enemies_remaining
    
    # Randomly selects from enemy_types list (defined lines 46-50):
    # enemy_types = [
    #     {"color": RED, "health": 30, "speed": 1.5, "reward": 20, "size": 12},  # line 47
    #     {"color": BLUE, "health": 50, "speed": 1.0, "reward": 40, "size": 12},  # line 48
    #     {"color": YELLOW, "health": 80, "speed": 0.8, "reward": 60, "size": 12} # line 49
    # ]
    enemy_type = random.choice(enemy_types)
    
    # Creates new enemy dictionary using:
    # - path_points[0] (starting position defined line 38)
    # - Enemy stats from chosen enemy_type
    enemy = {
        "x": path_points[0][0],  # First path X (line 38: (0, HEIGHT // 4))
        "y": path_points[0][1],  # First path Y
        "size": enemy_type["size"],  # From enemy_type (lines 47-49)
        "speed": enemy_type["speed"],
        "health": enemy_type["health"],
        "max_health": enemy_type["health"],
        "color": enemy_type["color"],
        "reward": enemy_type["reward"],
        "path_index": 0,  # Starts at first path segment
        "progress": 0  # 0-1 path progress
    }
    
    # Adds to global enemies list (defined line 52)
    enemies.append(enemy)
    
    # Updates counters used by:
    # - Wave tracking (lines 56-57)
    # - UI display (line 416 in draw_ui())
    # - Wave completion checks (lines 337-338)
    enemies_in_wave += 1
    enemies_remaining += 1
