...
def draw_towers():
    for tower in placed_towers:
        ...
        # Add wizard hat for wizard towers
        if tower["type"] == "wizard":
            hat_height = radius * 1.5
            hat_width = radius * 1.8
            hat_points = [
                (tower["x"] - hat_width/2, tower["y"] - radius),  # Left base
                (tower["x"] + hat_width/2, tower["y"] - radius),  # Right base
                (tower["x"], tower["y"] - radius - hat_height)    # Top point
            ]
...
def spawn_enemy():
    ...
    # If this is a boss wave (every 3rd wave) and we haven't spawned any enemies for this wave yet, then spawn the boss enemy.
    # for example, 3 % 3 == 0, 6 % 3 == 0, 9 % 3 == 0, but 4 % 3 == 1, 
    if current_wave % 3 == 0 and enemies_in_wave == 0:
        enemy = {
            ...
            "health": super_boss_stats["health"] * (1 + (current_wave // 3) * 0.5),
            "max_health": super_boss_stats["health"] * (1 + (current_wave // 3) * 0.5),
        ...
        for enemy_type in enemy_types:
            ...
            scaled_type["health"] = int(enemy_type["health"] * (1 + (current_wave - 1) * 0.1))
...
def update_towers():
    # Loop through all towers that have been placed on the map
    for tower in placed_towers:
        # Check if the tower is in cooldown (can't attack yet)
        if tower["cooldown"] > 0:
            # Reduce cooldown timer by 1 frame
            tower["cooldown"] -= 1
        else:
            # If tower is ready to attack, find the nearest enemy to this tower
            nearest_enemy = find_nearest_enemy(tower["x"], tower["y"])
            
            # Check if there's an enemy within the tower's attack range
            if nearest_enemy and math.dist((tower["x"], tower["y"]), (nearest_enemy["x"], nearest_enemy["y"])) <= tower["range"]:
                # Calculate the angle from tower to enemy for projectile direction
                angle = math.atan2(nearest_enemy["y"] - tower["y"], nearest_enemy["x"] - tower["x"])
                
                # Handle different tower types and their projectiles
                if tower["type"] == "dragon":
                    # Create a fire projectile for dragon towers
                    fire = {
                        "x": tower["x"],              # Starting x position
                        "y": tower["y"],              # Starting y position
                        "dx": projectile_speed * math.cos(angle),  # Horizontal speed component
                        "dy": projectile_speed * math.sin(angle),  # Vertical speed component
                        "damage": dragon_damage,      # Damage dealt by this projectile
                        "type": "fire",               # Projectile type
                        "radius": 12,                 # Size of the projectile
                        "life": 60,                  # How long the projectile lasts
                        "owner": tower                # Reference to the tower that fired it
                    }
                    # Add to fire projectiles list
                    fire_projectiles.append(fire)
                else:
                    # Create standard projectile for other tower types
                    projectile = {
                        "x": tower["x"],              # Starting x position
                        "y": tower["y"],              # Starting y position
                        "dx": projectile_speed * math.cos(angle),  # Horizontal speed component
                        "dy": projectile_speed * math.sin(angle),  # Vertical speed component
                        "damage": (cannon_damage if tower["type"] == "cannon" else wizard_damage),  # Damage based on tower type
                        "type": tower["type"],        # Projectile type matches tower type
                        "start_x": tower["x"],        # Starting position (for tracking distance)
                        "start_y": tower["y"]        # Starting position (for tracking distance)
                    }
                    # Add to regular projectiles list
                    projectiles.append(projectile)
                
                # Reset the tower's cooldown based on its type
...
def draw_ui():
    # Move the UI box down by 20 pixels from the previous adjustment (HEIGHT - 300 → HEIGHT - 280)
    ui_box_rect = pygame.Rect(WIDTH - 160, HEIGHT - 280, 150, 270)  # From 300 → 280 (20px down)
    ...
    # Adjust all text positions down by 20 pixels from previous adjustment
    gold_text = small_font.render(f"Gold: {gold}", True, GOLD)
    screen.blit(gold_text, (WIDTH - 150, HEIGHT - 270))  # From 290 → 270
    
    wave_text = small_font.render(f"Wave: {current_wave}", True, WHITE)
    screen.blit(wave_text, (WIDTH - 150, HEIGHT - 240))  # From 260 → 240
    
    enemies_text = small_font.render(f"Enemies: {enemies_remaining}", True, WHITE)
    screen.blit(enemies_text, (WIDTH - 150, HEIGHT - 210))  # From 230 → 210
    
    button_width, button_height = 130, 35
    # Adjust button positions down by 20 pixels from previous adjustment
    cannon_button_rect = pygame.Rect(WIDTH - 150, HEIGHT - 180, button_width, button_height)  # From 200 → 180
    wizard_button_rect = pygame.Rect(WIDTH - 150, HEIGHT - 140, button_width, button_height)  # From 160 → 140
    dragon_button_rect = pygame.Rect(WIDTH - 150, HEIGHT - 100, button_width, button_height)  # From 120 → 100
    range_button_rect = pygame.Rect(WIDTH - 150, HEIGHT - 60, button_width, button_height)    # From 80 → 60
    
    # (Rest of the button drawing code remains the same...)
