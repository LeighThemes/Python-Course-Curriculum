...
class Car:
    ...
    # MODIFY THE PARAMETERS OF OUR CODE TO ALLOW FOR INHERITANCE
    def __init__(self):

        self.width = 80
        self.height = 50
        # ADD NEW COLOR CODE HERE
...
class Sedan(Car): # TODO


class Truck(Car): # TODO


class SportsCar(Car): # TODO
...
class Ground:
    ...
    # ADD HILL WAVE GENERATION CODE HERE
...
class Game:
    ...
    def reset_game(self):
        ...
        if isinstance(self.selected_vehicle, Sedan):
            self.selected_vehicle = Sedan()
        elif isinstance(self.selected_vehicle, Truck):
            self.selected_vehicle = Truck()
        elif isinstance(self.selected_vehicle, SportsCar):
