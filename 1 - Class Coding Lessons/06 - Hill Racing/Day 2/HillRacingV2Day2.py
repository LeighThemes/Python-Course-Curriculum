...
class Car:
    def __init__(self):
        ...
        self.brake = 0.5
        self.max_speed = 18  # Increased max speed for more momentum
        self.fuel = 100
    ...
    def update(self, ground):
        ...
        if self.y + self.height // 2 > ground_height:
            ...
            if not self.in_air:
                ...
                # Apply ground friction to slow down the car
...
class Ground:
    ...
    def generate_waves(self):
        ...
        self.waves = []
        for _ in range(7):  # Generate 7 random sine waves
            amplitude = random.randint(20, 35)  # Random amplitude (larger range)
...
class Game:
    def __init__(self):
        ...
        self.coins = []
        self.fuel_cans = []
        self.coin_spawn_delay = 200  # Delay for spawning coins
        self.coin_spawn_timer = 0
        self.fuel_spawn_delay = 300  # Reduced delay for spawning fuel cans (more frequent)
    ...
    def spawn_coins_in_line(self):
        # Spawn a line of coins that curves along the hills and floats above the ground
        num_coins = random.randint(3, 6)  # Random number of coins in the line
        spacing = 50  # Spacing between coins
        start_x = WIDTH  # Start spawning at the right edge of the screen
        float_height = 20  # How much the coins float above the ground

        for i in range(num_coins):
            x = start_x + i * spacing
            y = self.ground.get_height_at(x) - Coin(0, 0).radius - float_height  # Curve along the hills and float
            self.coins.append(Coin(x, y))

    def spawn_fuel_can(self):
        # Spawn a fuel can either at the front or back of the coin line
        float_height = 20  # How much the fuel can floats above the ground
        if random.choice([True, False]):  # Randomly choose front or back
            x = WIDTH - 50  # Front of the line
        else:
            x = WIDTH + len(self.coins) * 50  # Back of the line

        y = self.ground.get_height_at(x) - FuelCan(0, 0).height - float_height  # Curve along the hills and float
        self.fuel_cans.append(FuelCan(x, y))
