...
bonus_font = pygame.font.Font(None, 72)
...
class Car:
    def __init__(self, width, height, color, max_speed, acceleration):
        ...
        self.min_grounded_time_for_jump = 120
        self.bonus_points = 0
        self.total_bonus_points = 0  # Track total bonus separately
        self.bonus_text_timer = 0
        self.bonus_text = ""
        self.bonus_text_pos = (0, 0)
    ...
    def update(self, ground):
        ...
        # Jump mechanic
        if (not self.in_air and 
            abs(self.speed_x) > self.jump_threshold and 
            (self.grounded_timer >= self.min_grounded_time_for_jump or isinstance(self, SportsCar))):
            slope = ground.get_slope_at(self.x)
    ...
    def draw_bonus_text(self):
        if self.bonus_text_timer > 0:
            alpha = min(255, self.bonus_text_timer * 5)
            bonus_surface = bonus_font.render(self.bonus_text, True, PURPLE)
            bonus_surface.set_alpha(alpha)
            screen.blit(bonus_surface, self.bonus_text_pos)
...
class SportsCar(Car):
    def __init__(self):
        ...
        self.min_airtime_for_spin = 30  # Starts spinning sooner
        self.air_bonus_counter = 0
    
    def update(self, ground):
        result = super().update(ground)
        
        # Sports car specific behavior - collect points while spinning in air
        if self.in_air and abs(self.angular_velocity) > 2:
            self.air_bonus_counter += 1
            if self.air_bonus_counter % 10 == 0:  # Add bonus every 10 frames
                bonus = 5
                self.bonus_points += bonus
                self.total_bonus_points += bonus  # Track total bonus
                self.bonus_text = f"+{bonus}"
                self.bonus_text_pos = (self.x - 50, self.y - 100)
...
class Game:
    ...
    def draw_hud(self):
        ...
        # Draw bonus points for sports car
        if isinstance(self.selected_vehicle, SportsCar) and self.selected_vehicle.bonus_points > 0:
            bonus_text = font.render(f"Air Bonus: {self.selected_vehicle.bonus_points}", True, PURPLE)
            screen.blit(bonus_text, (WIDTH - 200, 50))
    ...
    def draw_main_menu(self):
        ...
        title_text = title_font.render("Hill Climb Racing", True, BLACK)
        title_rect = title_text.get_rect(center=(WIDTH//2, HEIGHT//4))
        screen.blit(title_text, title_rect)
        
        play_button_hover = (WIDTH // 2 - 100 <= mouse_pos[0] <= WIDTH // 2 + 100 and
                            HEIGHT // 2 - 50 <= mouse_pos[1] <= HEIGHT // 2)
        quit_button_hover = (WIDTH // 2 - 100 <= mouse_pos[0] <= WIDTH // 2 + 100 and
                            HEIGHT // 2 + 25 <= mouse_pos[1] <= HEIGHT // 2 + 75)

        self.draw_button(WIDTH // 2 - 100, HEIGHT // 2 - 50, 200, 50, "Play", BLUE, (0, 0, 200), play_button_hover)
        self.draw_button(WIDTH // 2 - 100, HEIGHT // 2 + 25, 200, 50, "Quit", RED, (200, 0, 0), quit_button_hover)

    def draw_car_selection(self):
        screen.fill(SKY_BLUE)
        mouse_pos = pygame.mouse.get_pos()
        
        title_text = font.render("Select Your Vehicle", True, BLACK)
        title_rect = title_text.get_rect(center=(WIDTH//2, HEIGHT//4))
        screen.blit(title_text, title_rect)
        
        back_button_hover = (20 <= mouse_pos[0] <= 120 and 20 <= mouse_pos[1] <= 70)
        self.draw_button(20, 20, 100, 50, "Back", RED, (200, 0, 0), back_button_hover)
        ...
        # Highlight sports car as special
        sports_color = (255, 215, 0) if not sports_car_button_hover else (255, 255, 0)
        pygame.draw.rect(screen, sports_color, (WIDTH // 2 - 100, HEIGHT // 2 + 50, 200, 50))
        pygame.draw.rect(screen, BLACK, (WIDTH // 2 - 100, HEIGHT // 2 + 50, 200, 50), 2)
        sports_text = font.render("Sports Car", True, BLACK)
        sports_rect = sports_text.get_rect(center=(WIDTH // 2, HEIGHT // 2 + 75))
        screen.blit(sports_text, sports_rect)

    def draw_game_over(self):
        screen.fill(SKY_BLUE)
        game_over_text = title_font.render("Game Over", True, RED)
        screen.blit(game_over_text, (WIDTH // 2 - 140, HEIGHT // 5))
        
        score_text = font.render(f"Final Score: {self.score}", True, BLACK)
        screen.blit(score_text, (WIDTH // 2 - 90, HEIGHT // 2 - 25))
        
        # Show bonus points if sports car was used
        if isinstance(self.selected_vehicle, SportsCar):
            bonus_text = font.render(f"Air Bonus: {self.selected_vehicle.total_bonus_points}", True, PURPLE)
            screen.blit(bonus_text, (WIDTH // 2 - 80, HEIGHT // 2 - 90))
        
        mouse_pos = pygame.mouse.get_pos()
        menu_button_hover = (WIDTH // 2 - 100 <= mouse_pos[0] <= WIDTH // 2 + 100 and
                           HEIGHT // 2 + 25 <= mouse_pos[1] <= HEIGHT // 2 + 75)
        self.draw_button(WIDTH // 2 - 100, HEIGHT // 2 + 25, 200, 50, "Main Menu", GREEN, (0, 200, 0), menu_button_hover)
    ...
    def reset_game(self):
        ...
        if self.selected_vehicle:
            self.selected_vehicle.bonus_points = 0
            self.selected_vehicle.total_bonus_points = 0
    ...
    def run(self):
        ...
        while running:
                ...
                if event.type == pygame.QUIT:
                    ...
                    if self.current_state == MAIN_MENU:
                        if (WIDTH // 2 - 100 <= mouse_pos[0] <= WIDTH // 2 + 100 and
                            HEIGHT // 2 - 50 <= mouse_pos[1] <= HEIGHT // 2):
                            self.current_state = CAR_SELECTION
                        elif (WIDTH // 2 - 100 <= mouse_pos[0] <= WIDTH // 2 + 100 and
                              HEIGHT // 2 + 25 <= mouse_pos[1] <= HEIGHT // 2 + 75):
                            running = False
                    
                    elif self.current_state == CAR_SELECTION:
                        if (20 <= mouse_pos[0] <= 120 and 20 <= mouse_pos[1] <= 70):
                            self.current_state = MAIN_MENU
                        elif (WIDTH // 2 - 100 <= mouse_pos[0] <= WIDTH // 2 + 100 and
                              HEIGHT // 2 - 100 <= mouse_pos[1] <= HEIGHT // 2 - 50):
            ...
            if self.current_state == MAIN_MENU:
                ...
                self.score += self.selected_vehicle.bonus_points
                self.selected_vehicle.bonus_points = 0  # Reset current frame's bonus but keep total
                ...
                for coin in self.coins:
                    coin.draw()
                self.draw_hud()
                self.selected_vehicle.draw_bonus_text()
            elif self.current_state == GAME_OVER:
                self.draw_game_over()
