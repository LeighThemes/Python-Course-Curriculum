...
# Rainbow star settings
rainbow_star_size = 40
rainbow_star_spawn_delay = 15  # seconds
rainbow_star_spawn_timer = 0
rainbow_star = None
...
class RainbowStar:
    ...
    def draw(self):
        ...
        points = [
            ...
            (self.x + self.size * 0.75, self.y + self.size),
            (self.x + self.size * 0.25, self.y + self.size),
...
def spawn_rainbow_star():
    global rainbow_star
    rainbow_star = RainbowStar(random.randint(0, WIDTH - rainbow_star_size),
                               random.randint(0, HEIGHT // 2 - rainbow_star_size),
                               rainbow_star_size)

def update_shapes():
    global is_slow_motion, slow_motion_start_time
    current_time = time.time()
    if is_slow_motion and current_time - slow_motion_start_time >= slow_motion_duration:
        is_slow_motion = False
    speed = shape_speed * 0.25 if is_slow_motion else shape_speed  # Stronger slow motion (25% speed)

    for shape in shapes:
        # Apply slow motion to both horizontal and vertical movement
        shape["x"] += shape["dx"] * (0.25 if is_slow_motion else 1)  # Slow down horizontal movement
        shape["y"] += shape["dy"] * (0.25 if is_slow_motion else 1)  # Slow down vertical movement
        ...
        if shape["y"] + shape["size"] >= HEIGHT - floor_height:
            shape["dy"] *= -1  # Invert vertical speed to bounce
...
def check_collisions():
    global lives, flicker_start_time, score, second_cannon, second_cannon_start_time, hexagon, rainbow_star, is_slow_motion, slow_motion_start_time
    for shape in shapes[:]:
    ...
    for projectile in projectiles[:]:
        ...
        if hexagon and (hexagon["x"] < projectile["x"] < hexagon["x"] + hexagon_size and
            ...
            hexagon = None
            projectiles.remove(projectile)
        if rainbow_star and (rainbow_star.x < projectile["x"] < rainbow_star.x + rainbow_star.size and
                             rainbow_star.y < projectile["y"] < rainbow_star.y + rainbow_star.size):
            # Hit the rainbow star
            is_slow_motion = True
            slow_motion_start_time = time.time()
            rainbow_star = None
            projectiles.remove(projectile)
...
while running:
    if current_state == MAIN_MENU:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if (WIDTH // 2 - button_width // 2 <= mouse_pos[0] <= WIDTH // 2 + button_width // 2 and
                    ...
                    rainbow_star = None  # Reset rainbow star
        ...
        # Spawn rainbow star
        rainbow_star_spawn_timer += 1 / 60  # Increment by frame time
        if rainbow_star_spawn_timer >= rainbow_star_spawn_delay and not rainbow_star:
            spawn_rainbow_star()
            rainbow_star_spawn_timer = 0
